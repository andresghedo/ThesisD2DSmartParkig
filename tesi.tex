...
La documentazione per developer di Android[21] specifica che questo meccanismo è stato fatto per individuare, in una fase iniziale, peer con determinati servizi.
...
\begin{itemize}
\item \textit{isMyServiceRunning(Class<?> class, Context ctx)} : metodo che controlla se un determinato servizio background è attivo.
\item \textit{mergeEntries(ArrayList<ParkEvent> pe)} : metodo che unisce le nuove entries in input con quelle preesistenti nella memoria locale.
\item \textit{getMacAddr()} : metodo che ritorna l'indirizzo MAC del device.
\item \textit{isPossibleSync()} : metodo che sancisce se è possibile effettuare una sincronizzazione. -----codice
\end{itemize}
