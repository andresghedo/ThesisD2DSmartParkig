...
\part{Valutazione}
\chapter{Valutazione}
L'ultima fase riguarda la valutazione del processo di disseminazione delle informazioni, grazie alla quale riusciamo a studiare come le informazioni si propagano all'interno del mondo che si va a considerare.
...
\item \underline{\textit{sensor\_accuracy}}: accuratezza dei device Android nel performare l'evento di activity recognition rilascio o parcheggio. Misura espressa in probabilità da 0 a 1, compresi.
...
A questo punto, all'istante \textit{t+1}, se il device ha nella propria conoscenza locale 10 eventi $\textit{E\ped{i}\ap{loc}}$ dei 100 totali, avrà un'accuratezza del 10\%, se ne ha 50 avrà accuratezza del 50\% e così 
...
Per simulare la tecnologia presente nei sistemi \textit{V2V}, basato sull'\textit{802.11p}, si setta il \textit{range} di comunicazione sui 500m. 
...
La latenza di apertura dei canali di comunicazione viene settata dai 2 (\textit{min\_latency}) ai 10 secondi (\textit{max\_latency}). Si specifica che i parametri di \textit{min\_latency} e \textit{max\_latency}
...
Gli Smartphone Android, a partire dalla versione 4.0, danno la possibilità di comunicare tra loro secondo il paradigma Device-To-Device (\textit{D2D}). Il \textit{WiFi Direct} è la tecnologia che permette lo spreading 
...
Il meccanismo di disseminazione che è stato implementato nell'applicazione è stato reso totalmente automatico e trasparente all'utente. Una volta attivato permette di restare aggiornati sulla situazione parcheggi, 
...
Il meccanismo di disseminazione locale, oltre al problema del parcheggio, può essere facilmente adottato in altri ambiti (come ad esempio il marketing di prossimità).
