...
Nel momento in cui si scoprisse un servizio idoneo (indicante la presenza di un AccessPoint) viene mandato 
...
public ArrayList<Synchronization> getSyncByTimeRole(int apRole, String timeLimit) {
  ...
  if((apRole == -1)&&(!timeLimit.equals("ALL"))) {
    c = this.db.rawQuery(
          "SELECT * " + 
          "FROM " + SynchronizationEntrySQLiteDb.TABLE_NAME + " " + 
          "WHERE " + SynchronizationEntrySQLiteDb.COLUMN_SYNC_TIMESTAMP + " >= '" + timeLimit + "' " + 
          "ORDER BY " + SynchronizationEntrySQLiteDb.COLUMN_SYNC_TIMESTAMP + " DESC;", null);
