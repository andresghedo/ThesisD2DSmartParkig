...
Il \textit{V2V} riguarda comunicazioni tra veicoli in movimento portanto alla creazione di resti spontanee, denominate \textit{VANETs} (Vehicular ad hoc networks).
...
Il \textit{D2D} permette comunicazioni wireless distribuita tra device che sono nelle vicinanze. Questo approccio è quello che viene preso in considerazione nel lavoro di tesi, sfruttando i dispositivi utente per la 
...
\\L'implementazione riguarda la realizzazione del processo sopradescritto nel Sistema Operativo Android, con l'ausilio della tecnologia \textit{WiFi Direct}.
...
Il paragrafo illustra tutti gli attori principali che compongono il servizio di disseminazione delle informazioni. Si è cercato di spiegare, in ogni sottoparagrafo, le funzionalità
...
Dato che il processo di comunicazione tra peers deve avvenire in maniera completamente trasparente all'utente, è stato necessario implementare un servizio che svolgesse 
il meccanismo di invio e ricezione dati, anche nel caso in cui l'applicazione venisse chiusa.
Un Service è un componente Andorid, sprovvisto di interfaccia grafica, che si preoccupa di compiere procedure in background.
...
\begin{itemize}
 ...
 \item \textit{onStartCommand} : metodo che inizia l'esecuzione vera e propria del servizio di disseminazione, in background. Qui, sostanzialmente, espone in beaconing la 
  ...
  Si ricorda che l'attivazione del servizio è sancita dall'utente tramite la pressione di un pulstante da interfaccia grafica, come mostrano gli screenshots in figura 3.3.
 \item \textit{onDestroy} : Metodo che ferma il servizio e libera le strutture dati necessarie al processo di networking. L'utente richiama implicitamente questo metodo alla pressione del bottone di stop del servizio, 
 come mostrano gli screenshots in figura 3.3.
 \item \textit{startCheckNetworkTimer} : metodo privato, richiamato nell'\textit{onStartCommand}, che inizializza un oggetto Timer e lo schedula ogni 30s. Questi 30s possono essere interpretati logicamente come un time slot. 
 L'azione schedulata, prevede l'invio di un \textit{Intent}, in modo tale che il \textit{BroadcastReceiver} (definito nella sezione 3.1.8) lo possa catturare e controllare lo stato della macchina a stati finiti, 
 definita in fase di progettazione.
\end{itemize}
...
Infatti, la mansione principale di questa classe è proprio quella di esporre le istanze degli oggetti che permettono la realizzazione del processo di spreading.
Tramite questa è possibile accedere agli oggetti delle classi \textit{NetworkBeaconPeerSearcher}, \textit{NetworkAccessPoint} e\textit{ NetworkConnection}.
...
\begin{itemize}
\item \textit{startAccessPoint} / \textit{stopAccessPoint} : metodi che decretano l'inizio, o la fine, del device come Access Point.
\item \textit{startConnection} / \textit{stopConnection} : metodi che cercano di connettersi, o scollegarsi, da un HotSpot.
\item \textit{startServiceSearcher} / \textit{stopServiceSearcher} : metodi che sanciscono l'inizio, o la fine, della ricerca di beacon nelle vicinanze.
\end{itemize}
...
Ogni device, per decidere se fungere da Access Point o se connettersi come client ad un HotSpot già esistente, ha bisogno di comprendere ed essere avvisato della presenza o meno di altri host nell'ambiente locale.
Tutto quello che verrà illustrato in questa sezione è stato implementato nella classe \textit{NetworkBeaconPeerSearcher}.
\subsubsection{Scoperta di peers}
...
restituisce la lista dei peers presenti nei paraggi.
...
La documentazione per developer di Android[21] specifica che questo meccanismo è stato fatto per individuare, in una fase iniziale, peers con determinati servizi.
...
Questi pacchetti risultano comunque determinanti, in quanto incapsulano la stringa passphrase, necessaria ai client per connettersi in maniera automatica all'HotSpot esposto.
...
Nel momento in cui si scoprisse un servizio idoneo che indichi la presenza di un AccessPoint, in attesa di possibili sincronizzazioni, viene mandato 
...
A questo punto all'interno del metodo si richiama una procedura \textit{startBeaconAP}
 per creare ed esporre il servizio di notifica, in modo tale da che gli utenti esterni riescano a connettersi automaticamente all'HotSpot.
Oltre a questa operazione viene fatto partire il meccanismo per ricevere connessioni in input tramite Socket.
Come citato in precedenza, il beacon incorpora l'informazione di passphrase in modo tale che la connessione possa avvenire in maniera automatica tramite la classe \textit{NetworkConnection}.
...
Una volta che l'Access Point crea la rete ed altri peers riescono a connettersi, identificandosi con un indirizzo IP, si può passare all'apertura dei canali di comunicazione ed alla sincronizzazione degli 
...
L'Access Point, appena notificato dell'avvenuta creazione del gruppo, richiama la creazione di un oggetto della classe \textit{Server}, il quale istanzia una \textit{ServerSocket}, mettendosi in ascolto di eventuali richieste dai 
client, con il metodo \textit{accept()}.
I clients invece, quando notificati dal \textit{BroadcastReceiver} dell'avvenuta connessione all'HotSpot, creano un oggetto \textit{Socket} con l'indirizzo IP del Server e la porta idonea.
...
\newpage
\section{Dati}
\hyphenation{AppCompactActivity}
...
\item \textit{DisseminationFragment} : schermata in cui l'utente può attivare/disattivare il servizio di disseminazione tramite \textit{WiFi Direct}. Inoltre si forniscono all'utente informazioni sullo 
                              stato in cui si trova. Le figure 3.3 e 3.4 mostrano lo screenshot della schermata.
\item \textit{RecognitionFragment} : schermata in cui l'utente può attivare/disattivare le fasi di training e recognition della mobilità. Se la fase di recognition è attiva viene mostrata la predizione corrente. 
...
Per simulare la tecnologia presente nei sistemi \textit{V2V}, basato sull'\textit{802.11p}, si settano i parametri di \textit{range}, \textit{min\_latency} e \textit{max\_latency} in modo tale che 
il range di comunicazione sia piuttosto elevato, sui 500m, e che la \textit{min\_latency} e \textit{max\_latency} siano azzerate.
...
La tencologia \textit{Bluetooth} non riesce a raggiungere livelli di accuratezza soddisfacente. La latenza di connessione e soprattutto il basso range di comunicazione incidono negativamente sulla disseminazione.
