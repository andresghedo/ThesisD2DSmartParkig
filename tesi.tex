...
\\Come citato in [16] molti sistemi si basano su tecniche di \textit{VIP} (Video Image Processing) per dedurre l'occupazione o meno dei parcheggi. 
...
La comunicazione veicolare è stata pensata per propagare informazioni in maniera totalmente distribuita tra i vecoli, segnalando eventuali pericoli, incidenti, percorsi trafficati.
Le connessioni Vehicle-To-Vehicle richiedono componenti hardware e software che la maggior parte delle automobili in commercio non possiedono ancora.
Un altro modo di propagazione distribuita può avvenire invece tramite l'utilizzo di Smartphone, utilizzando il paradigma \textit{D2D} (Device-To-Device)[32][13].
Il \textit{D2D} permette comunicazioni wireless tra device che sono nelle vicinanze. L'approccio non prevede alcuna infrastrutture per lo scambio dati al di fuori dei dispositivi mobili (come ad esempio Smartphone e Tablet).
Questo approccio è quello che viene preso in considerazione nel lavoro di tesi, sfruttando i dispositivi utente per la propagazione di informazioni, relativa alla situazione parcheggi, all'interno della Smart City.
