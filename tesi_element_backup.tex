...
\end{figure}
\\\\Come si può comprendere dalla figura i device muovendosi continuamente all'interno dello scenario(identificato in questo caso con la griglia città), formeranno continuamente nuovi Hub, con attori sempre diversi. Questo 
...
Nella figura sono stati messi i componenti principali che concettualmente compongono l'architettura e come questi collaborano.
\begin{figure}[!h]
\centering
...
\item \textbf{Componente local DataBase:}
...
\begin{itemize}
\item \underline{cell\_id} : identificativo univoco della cella che la identifica all'interno della grid city. Informazione di tipo geografica.
\item \underline{event} : evento che può essere limitatamente PARKED o RELEASED. Indica se l'entry fa riferimento ad una sosta o alla liberazione di un parcheggio.
\item \underline{timestamp} : datetime che indica il momento in cui è stato performato l'evento in questione.
\item \underline{mac} : identificativo univoco del device che ha performato l'evento di parcheggio o rilascio.
\end{itemize}
La PRIMARY\_KEY è composta da \underline{\textless timestamp, mac\textgreater}. L'idea che vi sta dietro è quella che un utente può performare, in un dato istante, al più un unico 
evento di rilascio o parcheggio.
\\\\ Tabella \textbf{synchronizations}:
\begin{itemize}
\item \underline{timestamp} : datetime in cui è avvenuta la sincronizzazione
\item \underline{mac} : id univoco dell'altro device con cui è stata possibile la sincronizzazione
\item \underline{ap\_role} : booleano che indica se ero Access Point o meno al momento della sincronizzazione
\end{itemize}
...
Come si può notare dallo schema in figura XY, dallo stato STATE\_CONNECTION vi è anche la possibilità di ripartire dall'inizio, nel caso in cui, il device non riesca a connettersi all'HotSpot in un tempo ragionevole, 
...
\\Nella sezione screenshot, figura XZ, è possibile vedere quanto descritto.
...
\caption{Mappa del centro di Bologna divisa in celle, ognuna colorata con la relativa probabilità di trovare parcheggio}
\end{figure}
\begin{figure}[htbp]
\centering
...
\caption{Servizio di disseminazione attivo negli stati STATE\_ACCESS\_POINT\_NO\_PEERS e STATE\_ACCESS\_POINT\_PEERS}
\end{figure}
\begin{figure}[htbp]
\centering
...
\clearpage{\pagestyle{empty}\cleardoublepage}
\chapter{Valutazione}
\chapter{ALTRA SEZ}                 %crea la sezione
